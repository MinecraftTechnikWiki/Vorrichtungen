...
execute as @a if items entity @s weapon.* *[minecraft:custom_data~{EigDimS.3Sensor:true}] run tag @s add EtiDimS.3Ausgewaehlt
...
title @a[tag=EtiDimS.3Ausgewaehlt,predicate=dimensions-sensor:v3andere_dimension] actionbar ["",{"text":"Dimension == ","color":"gray","bold":true},{"text":"Unbekannt","color":"dark_purple","bold":true}]

# Damit die Schnellleiste wieder leer geräumt ist, wenn man den Stock nicht mehr ausgewählt hat, wird ein leerer Text angezeigt.
