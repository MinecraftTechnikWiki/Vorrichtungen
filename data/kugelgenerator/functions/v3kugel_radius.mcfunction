...
# Wenn der Markierer für die Hohlheit auf falsch eingestellt wurde, wird bei jedem Aufruf dieser Funktion ein Block für die Kugel platziert, ansonsten nur beim letzten Meter.
execute if entity @e[type=minecraft:marker,tag=EtiKugelG.3Hohl,name="falsch",sort=nearest,limit=1] if block ~ ~ ~ minecraft:air run setblock ~ ~ ~ minecraft:coarse_dirt replace
