...
execute at @s positioned ~5 ~-5 ~ run fill ~ ~ ~5 ~ ~ ~-5 minecraft:command_block[facing=up]{auto:true,Command:'summon minecraft:armor_stand ~ ~ ~ {Small:true,NoGravity:true,Tags:["EtiKugelG.2Alle","EtiKugelG.2Kugel"]}'} replace
execute at @s positioned ~5 ~-4 ~ run fill ~ ~ ~5 ~ ~ ~-5 minecraft:chain_command_block[facing=up]{auto:true,Command:"fill ~ ~-1 ~ ~ ~ ~ minecraft:air"} replace
