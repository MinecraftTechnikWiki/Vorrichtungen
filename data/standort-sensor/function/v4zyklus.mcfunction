...
execute at @e[type=minecraft:armor_stand,tag=EtiStaOS.4Sensor,tag=EtiStaOS.4Innen] positioned ~-10 ~ ~-10 as @a[dx=20,dy=5,dz=20] at @s positioned ~-5 ~-5 ~-5 unless entity @n[dx=10,dy=5,dz=10,type=minecraft:armor_stand,tag=EtiStaOS.4Sensor,tag=EtiStaOS.4Aussen] run title @s actionbar ["",{text:"Kubus(grün) UND (NICHT Kubus(grau)) == ",bold:true},{text:"wahr",color:"green",bold:true}]
