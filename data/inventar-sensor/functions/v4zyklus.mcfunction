...
execute at @a[tag=EtiInvS.4Ausgewaehlt,scores={PZInvS.4Drop=1..}] as @e[distance=..2,type=minecraft:item,sort=nearest,limit=1] store result score @s PZInvS.4Drop store result score VarInvS.4DropStapel PZInvS.4Drop run data get entity @s Item.count
...
execute at @a[tag=EtiInvS.4Ausgewaehlt,scores={PZInvS.4Drop=1..}] store success score VarInvS.4Ungleich PZInvS.4Drop run data modify storage inventar-sensor:v4daten "EigInvS.4SucheDrop" set from entity @e[distance=..2,type=minecraft:item,scores={PZInvS.4Drop=1..},sort=nearest,limit=1] Item

# Der fallen gelassene Drop wird von den Eigenschaften so geändert, dass er sofort wieder vom Spieler aufgehoben wird.
execute at @a[tag=EtiInvS.4Ausgewaehlt,scores={PZInvS.4Drop=1..}] run data merge entity @e[distance=..2,type=minecraft:item,sort=nearest,limit=1] {PickupDelay:0s}
