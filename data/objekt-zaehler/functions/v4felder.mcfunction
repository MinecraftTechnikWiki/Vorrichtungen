...
# Wenn der Name auf dem Schild nicht gefunden wurde, wird im Datenspeicher ein neuer leerer Eintrag gespeichert.
...
# Anschließend wird der Name aus dem Schild und die ID aus dem Markierer in den Datenspeicher kopiert.
execute if entity @s[tag=!EtiObjZae.4Gefunden] run data modify storage objekt-zaehler:v4daten "EigObjZae.4Felder"[0]."EigObjZae.4Name" set from block ~ ~ ~ front_text.messages[0]
